...
import {
  Activity, AlertCircle, CalendarDays, CheckSquare, ChevronDown,
  CircleDot, ClipboardList, Columns3, Filter, FolderKanban, GripVertical, LayoutDashboard,
  Link2, List, MessageSquare, MoreHorizontal, Paperclip, Plus, Search,
  UserPlus,
} from 'lucide-react';
...
function isOverdue(task: PMTask) {
  return Boolean(task.due_date && new Date(`${task.due_date}T23:59:59`).getTime() < Date.now());
}
...
function TaskDetailDialog({task,data,canWrite,onClose,onEdit,onDelete,onAddComment,onAddChecklist,onToggleChecklist}:{task:PMTask;data:{comments:PMComment[];checklist:PMChecklistItem[];activities:PMActivity[]}|null;canWrite:boolean;onClose:()=>void;onEdit:()=>void;onDelete:()=>void;onAddComment:(text:string)=>Promise<void>;onAddChecklist:(text:string)=>Promise<void>;onToggleChecklist:(id:string)=>Promise<void>}) { const [comment,setComment]=React.useState('');const [checklist,setChecklist]=React.useState('');return <Dialog isOpen onClose={onClose} title={task.title} size="xl">{canWrite&&<div className="mb-4 flex justify-end gap-2"><Button size="sm" variant="outline" onClick={onEdit}>Edit Task</Button><Button size="sm" variant="destructive" onClick={onDelete}>Delete Task</Button></div>}<div className="grid gap-6 lg:grid-cols-3"><div className="space-y-5 lg:col-span-2"><div><p className="text-[9px] font-bold uppercase text-muted-foreground">Description</p><p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed">{task.description||'No description.'}</p></div>{task.links.length>0&&<div><p className="mb-2 text-[9px] font-bold uppercase text-muted-foreground">Linked QA Records</p><div className="flex flex-wrap gap-2">{task.links.map((link)=><Link key={link.id} href={linkHref(link.link_type,link.linked_id)} className="flex items-center gap-1 rounded-lg border border-violet-500/20 bg-violet-500/10 px-2.5 py-1.5 text-[10px] font-bold text-violet-600"><Link2 className="h-3 w-3"/>{link.linked_code}</Link>)}</div></div>}<div><p className="mb-2 text-[9px] font-bold uppercase text-muted-foreground">Checklist</p><div className="space-y-2">{data?.checklist.map((item)=><label key={item.id} className="flex items-center gap-2 rounded-lg border border-border p-2.5 text-xs"><input type="checkbox" checked={item.is_done} disabled={!canWrite} onChange={()=>void onToggleChecklist(item.id)}/><span className={item.is_done?'text-muted-foreground line-through':''}>{item.text}</span></label>)}</div>{canWrite&&<form onSubmit={async(event)=>{event.preventDefault();if(!checklist.trim())return;await onAddChecklist(checklist);setChecklist('');}} className="mt-2 flex gap-2"><Input value={checklist} onChange={(event)=>setChecklist(event.target.value)} placeholder="Add checklist item" className="text-xs"/><Button size="sm">Add</Button></form>}</div><div><p className="mb-2 text-[9px] font-bold uppercase text-muted-foreground">Comments</p><div className="space-y-2">{data?.comments.map((item)=><div key={item.id} className="rounded-xl border border-border bg-secondary/30 p-3"><div className="flex justify-between"><span className="text-[10px] font-bold">{item.author_name}</span><span className="text-[8px] text-muted-foreground">{new Date(item.created_at).toLocaleString()}</span></div><p className="mt-1 text-xs">{item.body}</p></div>)}</div>{canWrite&&<form onSubmit={async(event)=>{event.preventDefault();if(!comment.trim())return;await onAddComment(comment);setComment('');}} className="mt-2 flex gap-2"><Input value={comment} onChange={(event)=>setComment(event.target.value)} placeholder="Write a comment" className="text-xs"/><Button size="sm">Comment</Button></form>}</div></div><aside className="space-y-5"><div className="rounded-xl border border-border p-4 text-xs"><Meta label="Reporter" value={task.reporter_name}/><Meta label="Priority" value={task.priority}/><Meta label="Due date" value={task.due_date||'Not set'}/><Meta label="Created" value={new Date(task.created_at).toLocaleDateString()}/><Meta label="Assignees" value={task.assignees.map((user)=>user.name).join(', ')||'Unassigned'}/><Meta label="Labels" value={task.labels.join(', ')||'None'}/>{task.attachment_url&&<a href={task.attachment_url} target="_blank" rel="noreferrer" className="mt-3 flex items-center gap-1 font-bold text-primary underline"><Paperclip className="h-3 w-3"/>{task.attachment_name||'Attachment'}</a>}</div><div><p className="mb-3 flex items-center gap-1.5 text-[9px] font-bold uppercase text-muted-foreground"><Activity className="h-3.5 w-3.5"/>Activity History</p><div className="space-y-3">{data?.activities.map((item)=><div key={item.id} className="border-l-2 border-border pl-3"><p className="text-[10px]"><span className="font-bold">{item.actor_name}</span> {item.details}</p><p className="mt-0.5 text-[8px] text-muted-foreground">{new Date(item.created_at).toLocaleString()}</p></div>)}</div></div></aside></div></Dialog>; }
